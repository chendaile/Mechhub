...
export const SidebarFooterUIState = ({
    ...
    const assignmentActions = useMemo<SidebarAssignmentAction[]>(() => {
        ...
        return candidates.filter(
            (action): action is SidebarAssignmentAction => typeof action.onClick === "function",
        );
    ...
    const assignmentsTitle = useMemo(() => {
        const hasStudentActions = assignmentActions.some((action) => action.audience === "student");

        const hasTeacherActions = assignmentActions.some((action) => action.audience === "teacher");
