...
import { authInstance, useSessionQuery, useProfileQuery, useAuthShowState } from "../../auth";
import { authzKeys, hasPermission, useMyAuthorizationQuery } from "../../authz";
import {
    chatUseCases,
    chatKeys,
    chatShareInterface,
...
import { ActiveChatTargetUIState } from "../states/ActiveChatTargetUIState";
import { SelectedClassUIState } from "../../class/ui/SelectedClassUIState";
import { AppShareFlow } from "../AppShareFlow";
import { AppView } from "../AppView";
import { PublishAssignmentCreationFlow } from "./PublishAssignmentCreationFlow";
...
export const AppShellUIState = () => {
    ...
    const { data: classContext, isLoading: isClassContextLoading } = MyClassContextQuery();
    ...
    const canAccessStudentAssignments = hasPermission(authorization, "assignment.student.access");

    const canAccessTeacherAssignments = hasPermission(authorization, "assignment.teacher.access");
    ...
    const myAssignmentsQuery = MyAssignmentsQuery(
        undefined,
    ...
    const myFeedbackQuery = MyFeedbackQuery(
        undefined,
        !!session && canAccessStudentAssignments,
        isViewFeedbackView
            ? {
            ...
            try {
                ...
                await Promise.all([
                    queryClient.invalidateQueries({
                        queryKey: assignmentKeys.feedbackDetail(viewerUserId, resolvedSubmissionId),
                    }),
                    queryClient.invalidateQueries({
                        queryKey: assignmentKeys.all(viewerUserId),
                    }),
    ...
    return {
        ...
        derived: {
            ...
            fallbackView,
            classOptions,
            teacherClassOptions: classOptions.filter((classItem) => classItem.role === "teacher"),
            selectedClass,
            classSessionGroups,
