...
export const ThinkingAutoOpenUIState = ({
    messages,
    isTyping,
    sessionId,
    scrollToBottom,
}: UseThinkingAutoOpenStateParams) => {
    const [autoOpenThinkingMessageId, setAutoOpenThinkingMessageId] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        if (lastAssistantStudyMessageId === previousAutoOpenMessageIdRef.current) {
            return;
        }
