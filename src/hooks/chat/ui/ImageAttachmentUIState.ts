...
export const ImageAttachmentUIState = () => {
    ...
    const removeAttachment = (id: string) => {
        setImageAttachments((previous) => previous.filter((attachment) => attachment.id !== id));
    };
    ...
    const totalImageBytes = useMemo(
        () => imageAttachments.reduce((total, attachment) => total + attachment.file.size, 0),
        [imageAttachments],
    );
    ...
    const isUploading = useMemo(
        () => imageAttachments.some((attachment) => attachment.isUploading || attachment.uploading),
        [imageAttachments],
    );
