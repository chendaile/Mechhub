...
import { TextCopyUIState } from "./TextCopyUIState";
...
export const TextMessageUIState = (text: string, options: UseTextMessageUiStateOptions = {}) => {
    const { isCopied, handleCopyText } = TextCopyUIState(text);
    const [thinkingOpen, setThinkingOpen] = useState(false);
    const thinkingContentRef = useRef<HTMLDivElement>(null);
    const [expandedAttachmentMap, setExpandedAttachmentMap] = useState<Record<number, boolean>>({});
    ...
    const isAttachmentExpanded = (index: number) => Boolean(expandedAttachmentMap[index]);
