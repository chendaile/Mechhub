import type { Class } from "./hooks/class/types";
...
export interface AppShellClassOption {
    id: string;
    name: string;
...
export const createClassOptions = (
    ...
    const classMap = new Map<string, AppShellClassOption>();

    teachingClasses.forEach((classItem) =>
        classMap.set(classItem.classId, {
    ...
    joinedClasses.forEach((classItem) => {
        if (!classMap.has(classItem.classId)) {
            classMap.set(classItem.classId, {
                id: classItem.classId,
                name: classItem.className,
                role: "student",
...
export const buildSharePickerDescription = (shareIntent: ShareIntent | null) => {
    ...
    if (shareIntent.kind === "chatMessage") {
        return "Current class integration only supports sharing a full private session.";
    }
